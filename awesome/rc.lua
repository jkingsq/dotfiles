...
mytextclock = wibox.widget.textclock("%a | %x | %X", 1)
...
-- }}}
function getIndexedTags(screen)
    local result = {}
...
function getFreeTag(screen)
    ...
    return tags[lowestIndex]
end
function snapMouse(c)
    local screen =  nil
    if client.focus then
        screen = client.focus.screen.geometry
    else
        screen = awful.screen.focused().geometry
    end
    if screen then
        -- move the mouse to the bottom-right corner of the current
        -- client
        local coords = {}
        local mousePos = mouse.coords()

        local top = screen.y
        local bottom = top + screen.height - 1

        local left = screen.x
        local right = left + screen.width - 1

        -- puts the mouse at different corners in this order:
        --  bottom-right
        --  top-left
        --  bottom-left
        --  top-right
...
--      awful.util.spawn( "xdotool mousemove -w " ..
--          client.window .. " " ..
--          client.width .. " " ..
--          client.height,
--          false)
    else
        awful.util.spawn( "xdotool mousemove_relative 0 -9999", false)
    end
end

-- {{{ Key bindings
globalkeys = gears.table.join(
    awful.key({ modkey, "Mod1"    }, " ", snapMouse),
    ...
    awful.key({ modkey, "Mod1"     }, "i",
        function ()
            ...
            local endTags = getEndTags(screen)
            if currentTag.index == endTags.max.index then
                endTags.min:view_only()
            else
                endTags.max:view_only()
            end
        end,
        {description = "focus last or first tag", group = "client"}
    ),
...
--            {description = "quit awesome", group = "awesome"}),
    ...
    awful.key({ modkey,           }, "b",
        function ()
            local dialog = function(stdout, stderr, reason, exit_code)
                naughty.notify({title = "Battery",
                                text = stdout,
                                timeout = 3,
                                replaces_id = 0})
            end
            awful.spawn.easy_async("acpi", dialog)
        end),
    awful.key({ modkey,           }, "d",
        function ()
            local callback = function(stdout, stderr, reason, exit_code) end
            awful.spawn.easy_async("/home/johnsk/.scripts/lastdownload.sh",callback)
        end),
    --fortune dialog
    -- awful.key({ modkey, "Mod1"    }, "f",
        -- function ()
            -- local dialog = function(stdout, stderr, reason, exit_code)
                -- naughty.notify({ title = "Fortune",
                                 -- text = stdout
                                 -- timeout = 15,
                                 -- replaces_id = 1})
            -- end
            -- awful.spawn.easy_async("fortune", dialog)
        -- end),
    --word salad dialog for making up passwords
    awful.key({ modkey, "Mod1"    }, "s",
        function ()
            local dialog = function(stdout, stderr, reason, exit_code)
                naughty.notify({ title = "Word Salad",
                                 text = stdout,
                                 timeout = 3,
                                 replaces_id = 3})
            end
            awful.spawn.easy_async("shuf -n 2 /usr/share/dict/words", dialog)
        end),
    --pavucontrol shortcut
    awful.key({ modkey,           }, "v",      function (c) awful.util.spawn("pavucontrol")  end),
    awful.key({ modkey,           }, "Print",  function (c) awful.util.spawn("scrot -e \"mv $f ~/Pictures\"") end)
)
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Mod1"    }, "o",
        function (c)
            local tag = getFreeTag(awful.screen.focused())
            if tag ~= nil then
                c:move_to_tag(tag)
            end
    ...
    awful.key({ modkey,           }, "n",
        function(c)
            c.opacity = (c.opacity > 0.3 and c.opacity - 0.3 or 1.00)
...
for k, v in pairs(autostart) do
    awful.spawn("pkill " .. v, function()end)
end
for k, v in pairs(autostart) do
    awful.spawn(v, function()end)
end
